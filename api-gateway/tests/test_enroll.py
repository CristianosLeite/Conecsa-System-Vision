...
# Generated by scripts/init.sh and gitignored: certificate files are never committed.
FIXTURES = os.path.join(os.path.dirname(__file__), "fixtures")


def _fixture_cert(name):
    path = os.path.join(FIXTURES, name)
    if not os.path.exists(path):
        pytest.skip(f"{name} is not generated (run scripts/init.sh where the hub CA builds)")
    with open(path, "rb") as f:
        return x509.load_pem_x509_certificate(f.read())


class TestHubIssuedCertificates:
    """Certificates issued by the real hub CA (rcgen), parsed with this pinned
    pyca/cryptography: the device must accept what the hub actually signs."""

    def test_a_hub_issued_leaf_parses_and_chains(self):
        from cryptography.x509.oid import ExtendedKeyUsageOID
        ca, leaf = _fixture_cert("hub_ca.pem"), _fixture_cert("hub_leaf.pem")
        extensions = leaf.extensions  # strict DER: raises on a malformed extension
        assert not any(isinstance(e.value, x509.BasicConstraints) for e in extensions)
        eku = extensions.get_extension_for_class(x509.ExtendedKeyUsage).value
        assert ExtendedKeyUsageOID.SERVER_AUTH in eku
        leaf.verify_directly_issued_by(ca)

    def test_an_explicit_ca_false_is_rejected(self):
        # Why the hub omits BasicConstraints on leaves: an explicit cA=FALSE is
        # a DER-encoded DEFAULT, which this parser refuses, and enrollment
        # would fail to install the certificate pair.
        leaf = _fixture_cert("hub_leaf_explicit_no_ca.pem")
