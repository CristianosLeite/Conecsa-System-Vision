...
class ProcessingPipelineService:
        ...
        while not self._stop.is_set():
            try:
                ...
                if (self._detection.is_running and self._detection.is_model_loaded()
                    ...
                    self._dispatch_detection(frame, seq)
                else:
    ...
    def _dispatch_detection(self, frame: np.ndarray, seq: int) -> None:
        ...
        the count; every other exit releases it.
        """
        ...
        try:
            ...
            handed = self._handoff(
                self._q_infer, (seq, generation, frame, input_data, meta))
        finally:
            if not handed:
                self._end_inflight()
    ...
    def _stage_infer(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            handed = False
            try:
                seq, generation, frame, input_data, meta = item
                output_data, inference_time = self._detection.infer(input_data, generation)
                handed = self._handoff(
                    self._q_finish,
                    (seq, generation, frame, output_data, meta, inference_time))
    ...
    def _stage_finish(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if item is None:
                continue
            try:
                seq, generation, frame, output_data, meta, inference_time = item
